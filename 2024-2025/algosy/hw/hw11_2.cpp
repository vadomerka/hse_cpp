...
private:
  ...
  void splitChild(Node *parent, int childIndex) {
    ...
    // Создаем новый узел для правой половины расщепляемого узла
    Node *newChild = new Node(child->isLeaf);

    // Серединный ключ перемещается в родительский узел
    parent->keys.insert(parent->keys.begin() + childIndex, child->keys[1]);

    // Переносим ключи из правой части узла в новый узел
    newChild->keys.push_back(child->keys[2]);

    // Если это не лист, переносим также соответствующих детей
    if (!child->isLeaf) {
      newChild->children.push_back(child->children[2]);
      newChild->children.push_back(child->children[3]);
    }

    // Обновляем исходный (ранее заполненный) узел
    child->keys.resize(1);
    if (!child->isLeaf) {
      child->children.resize(2);
    }

    // Добавляем новый узел в родителя
  ...
  void insertNonFull(Node *node, int key) {
    if (node->isLeaf) {
      ...
      // Найти правильного потомка для вставки
      int i = 0;
      while (i < node->keys.size() && key > node->keys[i]) {
        i++;
      }

      // Расщепить потомка, если он полон
      ...
      // Рекурсивно вставить в подходящего потомка
